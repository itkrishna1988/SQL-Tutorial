...
These tables use a clustered columnstore index

dbo.bigProductCCSI
dbo.bigTransactionHistoryCCSI
...
ALTER TABLE bigProductCCSI
ALTER COLUMN ProductId INT NOT NULL	
GO


CREATE CLUSTERED COLUMNSTORE INDEX [pk_bigProductCCSI] 
ON [dbo].[bigProductCCSI] WITH (DROP_EXISTING = OFF);
GO
...
INTO bigTransactionHistoryCCSI
FROM
(
	SELECT
		p.ProductID, 
...
ALTER TABLE bigTransactionHistoryCCSI
ALTER COLUMN TransactionID INT NOT NULL
GO


CREATE CLUSTERED COLUMNSTORE INDEX [pk_bigTransactionHistoryCCSI] 
ON [dbo].[bigTransactionHistoryCCSI] WITH (DROP_EXISTING = OFF);
GO
