...
select A.name, last_execution_time, creation_time, total_seconds, total_elapsed_time/C.Total_Execution_count/1000000.0 average_seconds_new,
average_seconds,
 execution_count,
Disk_Reads, Memory_Reads, Memory_Writes, CPU_Time,C.Total_Execution_count
from 
(
...
qs.total_elapsed_time,
 qs.total_worker_time as CPU_Time, execution_count,
...
where qt.dbid = DB_ID() and last_execution_time >= '6/9/2013'
) a JOIN

(
SELECT MAX(Rank1) Total_Execution_count,name FROM 
(
SELECT  dense_rank() over(partition by o.object_id, convert(date,last_execution_time) order by qs.total_elapsed_time desc) as Rank1, 
o.name
FROM sys.dm_exec_query_stats qs
